	...
	AActor* HomingTarget, bool bOverridePitch, float PitchOverride)
{
	...
	for (const FRotator& Rot : Rotations)
	{
		...
		{
			// 如果撞到地板或墙壁，则鼠标点击位置不一定是该Actor的RootComponent位置
			// 所以创建一个新的USceneComponent，但是HomingTargetComponent是一个weak ptr，
			// 所以当Projectile销毁时，新创建的Scene组件不会销毁，所以将其创建在Projectile中，
			// 等Projectile销毁时，这个创建的Scene组件就会被回收
